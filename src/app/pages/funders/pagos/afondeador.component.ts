...
@Component({
  selector: 'app-afondeador',
  templateUrl: './afondeador.component.html',
  styles: []
})
export class AfondeadorComponent implements OnInit {
  ...
  ngOnInit() {
   ...
   // this.getAcceso('/pagos/afondeador');
  ...
  //  console.log(this.financieras);
    for ( const prop in this.financieras ) {
      ...
      //  console.log(this.financieras[prop]);
        this.correo = this.financieras[prop].email_contribuyente;
        this.contributor_to_id = this.financieras[prop].id_contribuyente;
        this.contributor_from_id = this.financieras[prop].id_cont_finan;
        break;
 ...
 swal2.fire({
  ...
  //  console.log(valormoneda);
    this._fundersservice.getcompanyfacturaspayment( (document.getElementById('cadena') as HTMLInputElement).value, valormoneda ).subscribe( resp => {
                                                                                              swal2.close();
